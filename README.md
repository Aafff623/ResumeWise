...
```bash
# 终端 1：静态页
cd docs/output/reports/ui-prototype/html-src
...
3. 模型选 **DeepSeek V4 Flash**（`deepseek-v4-flash`）或 **V4 Pro**（`deepseek-v4-pro`）  
4. 填写简历 → 开始 AI 诊断（结果由模型动态生成）

### 业务前后端（规划中）

> `resume-web/` · `resume-api/` 待业务 PRD 批准后按原型复刻（Vue3 + Spring Boot）。
...
```bash
# 后端
cd resume-api && mvn spring-boot:run   # :8080，默认 AI Mock

# 前端
cd resume-web && pnpm install && pnpm dev   # :5173
```
...
| 层 | 技术 |
|---|---|
| 原型前端 | HTML / CSS / JS · Markdown 预览 · localStorage |
| 真实 AI | DeepSeek V4 Flash / Pro · 本机 CORS 代理 |
| 规划前端 | Vue3 · Vite · Element Plus · Axios |
| 规划后端 | Spring Boot 3 · REST · 无数据库 |
...
├── docs/
│   ├── adr/ · agents/ · contexts/ · images/
│   └── output/reports/ui-prototype/html-src/   # 静态原型（可演示）
└── resume-web/ · resume-api/                   # 规划中
```
...
| [`docs/output/reports/ui-prototype/html-src/`](docs/output/reports/ui-prototype/html-src/) | 原型入口与 DESIGN_SPEC |
